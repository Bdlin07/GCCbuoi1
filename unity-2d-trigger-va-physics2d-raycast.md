...
0. [Ôn lại bài cũ – Buổi 4](#0-ôn-lại-bài-cũ--buổi-4)
...
## 0. Ôn lại bài cũ – Buổi 4

Trước khi học Trigger và Raycast, mình ôn nhanh những phần nền tảng đã học ở buổi 4: New Input System, Physics 2D, va chạm, các loại `Rigidbody2D` và cách di chuyển nhân vật.

### 0.1. New Input System

New Input System là cách Unity nhận thao tác từ bàn phím, chuột hoặc tay cầm. Điểm dễ hiểu nhất là: mình đặt tên cho **hành động** trước, sau đó mới gán phím hoặc nút cho hành động đó.
...
1. Mở `Window > Package Manager` và cài package `Input System`.
2. Trong cửa sổ Project, tạo một Input Actions Asset.
3. Mở asset và tạo Action Map tên `Gameplay`.
4. Tạo Action `Move` với kiểu `Value/Vector2`.
5. Gán WASD cho `Move` bằng binding `2D Vector Composite`.
6. Tạo Action `Jump` với kiểu `Button`.
7. Gán phím Space cho `Jump`.

Nói ngắn gọn:

- `Move` trả về một `Vector2` để mình biết hướng di chuyển.
- `Jump` chỉ cần cho biết nút vừa được nhấn hay chưa.
...
> Thường đọc input trong `Update`. Nếu nhân vật dùng `Rigidbody2D`, hãy áp dụng chuyển động vật lý trong `FixedUpdate`.

`Update` bám theo khung hình hiển thị, nên thích hợp để bắt thao tác người chơi. `FixedUpdate` bám theo bước mô phỏng vật lý, nên thích hợp để thay đổi vận tốc hoặc thêm lực.

### 0.2. Physics 2D

Physics 2D là hệ thống giúp vật thể rơi, va chạm, trượt, nảy và bị lực tác động. Hai component xuất hiện nhiều nhất là `Rigidbody2D` và `Collider2D`.

| Thành phần | Công việc chính |
|---|---|
| `Rigidbody2D` | Quản lý chuyển động vật lý, vận tốc, trọng lực và khối lượng |
| `Collider2D` | Tạo hình dùng để phát hiện va chạm |
| `Physics Material 2D` | Chỉnh ma sát và độ nảy |

Điểm dễ nhầm:

> `Rigidbody2D` không tự tạo hình va chạm. Muốn chạm được vật khác thì GameObject vẫn cần có `Collider2D`.

Sprite chỉ là hình ảnh để mình nhìn thấy. Hai sprite chồng lên nhau chưa có nghĩa là hệ vật lý biết chúng đang va chạm.

### 0.3. `Update` và `FixedUpdate`

`Update` chạy theo từng khung hình. Máy chạy được nhiều FPS thì `Update` được gọi nhiều lần hơn.

`FixedUpdate` chạy theo nhịp vật lý cố định. Vì vậy, cách chia công việc thường dùng là:

```csharp
private void Update()
{
    // Đọc thao tác người chơi.
    moveInput = moveAction.action.ReadValue<Vector2>();
}
...
Không nên hiểu cứng nhắc rằng `Update` không bao giờ được gọi physics query hoặc `FixedUpdate` không bao giờ được đọc dữ liệu khác. Ý chính là input và vật lý chạy theo hai nhịp khác nhau, nên mình tách chúng ra để chuyển động ổn định hơn.

### 0.4. Va chạm và điều kiện để va chạm
...
- Hai collider đang thật sự chạm nhau, không chỉ có hai sprite chồng lên nhau.
- Cả hai đều dùng component 2D.
- Layer Collision Matrix không chặn cặp layer đó.
- Nếu muốn va chạm cứng bình thường thì `Is Trigger` phải tắt.

#### Các hàm collision

| Hàm | Khi được gọi |
|---|---|
| `OnCollisionEnter2D` | Khi hai collider vừa bắt đầu chạm |
| `OnCollisionStay2D` | Trong lúc hai collider vẫn đang chạm |
| `OnCollisionExit2D` | Khi hai collider không còn chạm nhau |
...
Ở đây tham số nhận vào là `Collision2D`, không phải `Collider2D`. Từ `Collision2D`, mình có thể đọc collider, GameObject và thông tin các điểm tiếp xúc.

### 0.5. `Rigidbody2D`: Dynamic, Kinematic và Static

| Body Type | Hiểu đơn giản | Ví dụ |
|---|---|---|
| Dynamic | Bị trọng lực, lực và va chạm tác động | Player, quái, thùng, bóng |
| Kinematic | Thường do code điều khiển, không bị trọng lực tác động như Dynamic | Platform di động, cửa |
| Static | Đứng yên và dùng làm môi trường | Nền, tường |

Cách chọn nhanh:

- Vật cần phản ứng vật lý: dùng `Dynamic`.
- Vật di chuyển theo đường được viết sẵn: dùng `Kinematic`.
- Vật không di chuyển: dùng `Static` hoặc chỉ để collider tĩnh.

Một số lưu ý:

- Không nên thay đổi `transform.position` liên tục cho `Rigidbody2D Dynamic`.
- `Dynamic` thường tương tác với Dynamic, Kinematic và Static.
- Kinematic mặc định chủ yếu tương tác vật lý với Dynamic.
- Platform di động thường dùng Kinematic kết hợp `MovePosition` trong `FixedUpdate`.

### 0.6. Các cách di chuyển nhân vật

#### Cách 1: thay đổi Transform

```csharp
transform.position += Vector3.right * speed * Time.deltaTime;
```

Cách này đơn giản nhưng không phù hợp khi Player dùng `Rigidbody2D Dynamic`, vì có thể làm vật xuyên tường hoặc bỏ qua một phần xử lý vật lý.

Nó hợp hơn với object không tham gia physics hoặc những chuyển động trang trí đơn giản.
...
Cách này dễ điều khiển, phản hồi nhanh và vẫn làm việc với hệ va chạm. Đây là lựa chọn dễ dùng cho nhân vật platformer.

Ta chỉ thay vận tốc X và giữ nguyên vận tốc Y để trọng lực và hành động nhảy vẫn hoạt động.
...
Cách này phù hợp với `Rigidbody2D Kinematic`, ví dụ platform di động hoặc NPC đi theo đường cố định.
...
`AddForce` tạo cảm giác có quán tính và tự nhiên hơn, nhưng khó dừng chính xác. Nó hợp với bóng, tàu hoặc game thiên về vật lý.
...
### 0.7. Ví dụ Player 2D ngắn gọn

Ví dụ dưới đây kết hợp New Input System, `Rigidbody2D`, di chuyển ngang và Raycast kiểm tra mặt đất trước khi nhảy:
...
public class SimplePlayer2D : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Dùng |= để không làm mất lần nhấn xảy ra giữa hai FixedUpdate.
        jumpPressed |= jumpAction.action.WasPressedThisFrame();
    }
...
```text
Input tạo ra hướng di chuyển
        ↓
Rigidbody2D thực hiện chuyển động
        ↓
Raycast kiểm tra chân có chạm layer Ground không
        ↓
Đang chạm đất và vừa bấm Jump thì mới cho nhảy
```

Nếu phiên bản Unity đang dùng chưa có `Rigidbody2D.linearVelocity`, thay `linearVelocity` bằng `velocity`.
